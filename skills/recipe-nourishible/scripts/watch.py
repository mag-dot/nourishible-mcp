...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="BROWSER",
        help="Read cookies from an installed browser (chrome, firefox, safari, edge, "
             "brave, ...) for login-sensitive sources like Instagram Reels/Stories. "
             "Default: WATCH_COOKIES_FROM_BROWSER in ~/.config/watch/.env.",
    )
    ap.add_argument(
        "--cookies",
        type=str,
        ...
        metavar="FILE",
        dest="cookies_file",
        help="Path to a Netscape-format cookies.txt file (takes precedence over "
             "--cookies-from-browser). Default: WATCH_COOKIES_FILE in ~/.config/watch/.env.",
    )
    args = ap.parse_args()
    ...
    cookies_file = args.cookies_file or config.get("cookies_file")
