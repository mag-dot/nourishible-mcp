...
- `--cookies-from-browser BROWSER` — read cookies from an installed, logged-in browser (`chrome`, `firefox`, `safari`, `edge`, `brave`, …). Needed for login-sensitive sources — see "Instagram & login-sensitive sources" below. Default: `WATCH_COOKIES_FROM_BROWSER` in `~/.config/watch/.env`.
- `--cookies FILE` — path to a Netscape-format `cookies.txt` (exported via a browser extension). Takes precedence over `--cookies-from-browser`. Default: `WATCH_COOKIES_FILE` in `~/.config/watch/.env`.

## Instagram & login-sensitive sources

`yt-dlp` supports Instagram URLs the same way it supports YouTube/TikTok/etc., but Instagram is aggressive about blocking anonymous requests — public profile grids often work, but **Reels and Stories frequently 401 or "login required" without an authenticated session.**

If a `/watch` run against an `instagram.com` URL fails to produce a video or captions, the script now prints a hint pointing at cookie auth. Two ways to provide it:

1. **From a browser (simplest).** Stay logged into Instagram in a regular browser, then either set it once in `~/.config/watch/.env`:
   ```
   WATCH_COOKIES_FROM_BROWSER=chrome
   ```
   or pass it per-call: `/watch https://www.instagram.com/reel/... --cookies-from-browser chrome`. Any yt-dlp-supported browser works (`chrome`, `firefox`, `safari`, `edge`, `brave`, `chromium`, `vivaldi`, `opera`).
2. **From an exported cookies file.** Export Instagram cookies (Netscape format) with a browser extension, then set `WATCH_COOKIES_FILE=/path/to/cookies.txt` or pass `--cookies /path/to/cookies.txt`.

Notes:
- Cookies are read locally by `yt-dlp` and sent only to `instagram.com` as part of the normal authenticated request — this skill never stores or transmits them elsewhere.
- If the browser is open while `--cookies-from-browser` runs, some browsers (notably Chrome) lock the cookie DB; close the browser or use an exported `cookies.txt` if extraction fails.
- Public, non-Reel Instagram posts sometimes work without cookies — try without auth first if you'd rather not use it.
